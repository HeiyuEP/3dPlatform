...
#include <fstream>
#include <chrono>
#include "/home/zhike/公共的/c30041999/code/open-dibr/include/stb_image/stb_image.h"
#include "GenerateDepthImg.h"
extern void saveImage(unsigned char* image, int width, int height, bool saveAsPNG, int frameNr, std::string outputPath);
extern int dibr_test(uint8_t* colorData, uint8_t* depthData, uint8_t** resultImg, bool outputLeft);
extern int dibr_flag_test(uint8_t* colorData, uint8_t* depthData, uint8_t** resultImg, bool outputLeft, uint8_t flag);
// cyf test👆
...
namespace video {
  ...
  // std::vector<Ort::Value> preProcess(int height, int width) {
  //   height = 1092;
  //   width = 1932;
  ...
  void generateDepthImg(int imgHeight, int imgWidth, std::uint8_t *srcData, std::uint8_t *src3Data, std::uint8_t *depthData) {
    Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "my_test");
    Ort::SessionOptions session_options;
    ...
    const char* modelPath = "/home/zhike/公共的/x30032372/Depth-Anything-V2/vits4.onnx";
    Ort::Session session(env,modelPath,session_options);
    auto time2 = std::chrono::high_resolution_clock::now();

    //2、read pic
    cv::Mat image(imgHeight, imgWidth, CV_8UC4, srcData);
    cv::cvtColor(image, image, cv::COLOR_RGBA2RGB);
    memcpy(src3Data, image.data, imgWidth*imgHeight*3);
    int height = 1442;
    int width = 2562;
    cv::resize(image,image,cv::Size(width,height));
    ...
    std::chrono::duration<double, std::milli> timeCost2 = time4-time3;
    std::cout << "loadmodel time: " << timeCost1.count() << " ms\n";
    std::cout << "run time: " << timeCost2.count() << " ms\n";
    ...
    cv::resize(outputImage,outputImage,cv::Size(imgWidth,imgHeight));
    memcpy(depthData, outputImage.data, imgWidth*imgHeight);
  }

  int depthAnythingV2() {
    const char* scriptPath = "/home/zhike/公共的/x30032372/Depth-Anything-V2/test.py";
    std::cout << "Calling script: " << scriptPath << std::endl;
 
    // 构建命令字符串
    std::string command = std::string("python3 ") + scriptPath;
  ...
  void setAlphaToOne(uint8_t* img, size_t count) {
    for (size_t i=3; i<count ; i+=4) {
      img[i] = 255;
    }
  }

  void printRGBA(uint8_t* img, size_t count) {
    for (size_t i=0; i<count ; ++i) {
      std::cout << "Value " << i+1 << ": "
                << static_cast<int>(img[i]) << " ";
      if ((i+1)%4==0) {
        std::cout << std::endl;
      }
    }
  ...
  void generateLeftRightImg() {
    std::string exe_path = "/home/zhike/公共的/c30041999/code/open-dibr/bin/RealtimeDIBR";
    std::string input_path = "/home/zhike/公共的/c30041999/code/sunshine/asset";
    std::string json_path = "/home/zhike/公共的/c30041999/code/sunshine/asset/example_opengl.json";
    std::string output_left = "/home/zhike/公共的/c30041999/code/sunshine/asset/left_omaf.json";
    std::string output_right = "/home/zhike/公共的/c30041999/code/sunshine/asset/right_omaf.json";
    std::string dibr_left = "/home/zhike/公共的/c30041999/code/sunshine/asset";
    std::string dibr_right = "/home/zhike/公共的/c30041999/code/sunshine/asset";

    std::string command_left = exe_path + " -i " + input_path + 
                                " -j " + json_path +
                                " -p " + output_left +
                                " -o " + dibr_left +
                                " --triangle_deletion_margin 10";

    std::string command_right = exe_path + " -i " + input_path + 
                                " -j " + json_path +
                                " -p " + output_right +
                                " -o " + dibr_right +
                                " --triangle_deletion_margin 10";

    int result_left = std::system(command_left.c_str());
    if (result_left == -1) {
      std::cerr << "Error executing left command." << std::endl;
    }

    int result_right = std::system(command_right.c_str());
    if (result_right == -1) {
      std::cerr << "Error executing right command." << std::endl;
    }
  }

  cv::Mat* concatenateImages_exeVersion() {
    // read img
    cv::Mat img1 = cv::imread("/home/zhike/公共的/c30041999/code/sunshine/asset/output_left.png");
    cv::Mat img2 = cv::imread("/home/zhike/公共的/c30041999/code/sunshine/asset/output_right.png");
    ...
    if (img1.rows != img2.rows) {
      std::cerr << "Images must have the same height!" << std::endl;
      return nullptr;
    }

    // concat
    cv::Mat* concatenated = new cv::Mat();
    cv::hconcat(img1, img2, *concatenated);
    // cv::imwrite("/home/zhike/公共的/c30041999/code/sunshine/asset/o.png",*concatenated);
    printf("concated images rgba:\n");
    printRGBA((*concatenated).data, 100);

    return concatenated;
  }

  void concatenateAndFilpImages(uint8_t* data1, uint8_t* data2, int width, int height, int channels, uint8_t** new_data) {
  ...
  void resizeImage(uint8_t* input_data, int input_width, int input_height, int channels, uint8_t** output_data, int output_width) {
  ...
  captureThread(
    ...
    //std::uint8_t *data = new uint8_t[size];
    printf("new uint8_t[size]:");
    platf::adjust_thread_priority(platf::thread_priority_e::critical);

    std::chrono::duration<double, std::milli> timeCost1;
    std::chrono::duration<double, std::milli> timeCost2;
    std::chrono::duration<double, std::milli> timeCost3;
    std::chrono::duration<double, std::milli> timeCost4;
    std::chrono::duration<double, std::milli> timeCost5;
    
    // // cyf test
    int leftInitRet = dibr_flag_test(nullptr, nullptr,nullptr,1,0);
    int rightInitRet = dibr_flag_test(nullptr, nullptr,nullptr,0,0);
    ...
    while (capture_ctx_queue->running()) {
      ...
      printf("111111\n");
      //printf("img w: %d, img h: %d\n",imgs.front().width,imgs.front().height);
      auto push_captured_image_callback = [&](std::shared_ptr<platf::img_t> &&img, bool frame_captured) -> bool {
        ...
        KITTY_WHILE_LOOP(auto capture_ctx = std::begin(capture_ctxs), capture_ctx != std::end(capture_ctxs), {
          ...
          // printf("img w: %d, img h: %d\n",img->width,img->height);
          // cv::Mat image(img->height, img->width, CV_8UC4, img->data);
          // setAlphaToOne(img->data, img->height*img->width*4);
          // cv::imwrite("/home/zhike/公共的/x30032372/Depth-Anything-V2/ok88.png",image);
          if (frame_captured) {
            ...
            /*
            {
              ...
              size_t srcSize = img->width*img->height*4;
              std::uint8_t *srcData = new uint8_t[srcSize];
              if(img->data!=NULL){
                memcpy(srcData, img->data, srcSize);
              }

              size_t src3Size = img->width*img->height*3;
              std::uint8_t *src3Data = new uint8_t[src3Size];

              size_t depthSize = img->width*img->height;
              std::uint8_t *depthData = new uint8_t[depthSize];

              generateDepthImg(img->height,img->width,srcData,src3Data,depthData);

              auto timeB = std::chrono::high_resolution_clock::now();
              uint8_t* left3dResult = NULL;
              int ret_left = dibr_test(src3Data, depthData, &left3dResult, true);
              printf("left ret_left:%d\n", ret_left);

              uint8_t* right3dResult = NULL;
              int ret_right = dibr_test(src3Data, depthData, &right3dResult, false);
              printf("right ret_right:%d\n", ret_right);

              auto timeC = std::chrono::high_resolution_clock::now();
              if (ret_left == 0 && ret_right == 0 && left3dResult != NULL && right3dResult != NULL) {
                uint8_t* concatedImgData = nullptr;
                concatenateAndFilpImages(left3dResult, right3dResult, img->width, img->height, img->pixel_pitch, &concatedImgData);
                auto timeD = std::chrono::high_resolution_clock::now();

                uint8_t* resizedImgData = nullptr;
                resizeImage(concatedImgData, img->width*2, img->height, img->pixel_pitch, &resizedImgData, img->width);
                auto timeE = std::chrono::high_resolution_clock::now();

                memcpy(img->data, resizedImgData, img->width*img->height*img->pixel_pitch);
                auto timeF = std::chrono::high_resolution_clock::now();

                timeCost1 = timeB - timeA;
                timeCost2 = timeC - timeB;
                timeCost3 = timeD - timeC;
                timeCost4 = timeE - timeD;
                timeCost5 = timeF - timeE;
                std::cout << "Elapsed time: " << timeCost1.count() << " ms\n";
                std::cout << "Elapsed time: " << timeCost2.count() << " ms\n";
                std::cout << "Elapsed time: " << timeCost3.count() << " ms\n";
                std::cout << "Elapsed time: " << timeCost4.count() << " ms\n";
                std::cout << "Elapsed time: " << timeCost5.count() << " ms\n";
            ...
            */
            
            {
              ...
              auto timeB = std::chrono::high_resolution_clock::now();
              uint8_t* left3dResult = NULL;
              int ret_left = dibr_flag_test(src3Data, depthData,&left3dResult,1,1);
              printf("left ret_left:%d\n", ret_left);

              uint8_t* right3dResult = NULL;
              int ret_right = dibr_flag_test(src3Data, depthData,&right3dResult,0,1);
              ...
              if (ret_left == 0 && ret_right == 0 && left3dResult != NULL && right3dResult != NULL) {
                ...
                std::cout << "【INFO】Generate Depth Image: " << timeCost1.count() << " ms" << std::endl;
                std::cout << "【INFO】Generate Left&Right Image: " << timeCost2.count() << " ms" << std::endl;
                std::cout << "【INFO】Concat and filp Left&Right Image: " << timeCost3.count() << " ms" << std::endl;
                std::cout << "【INFO】Resize concated Image: " << timeCost4.count() << " ms" << std::endl;
                std::cout << "【INFO】Memcpy to Ori Image Address: " << timeCost5.count() << " ms" << std::endl;
                std::cout << "-----------------------------------------" << std::endl;
