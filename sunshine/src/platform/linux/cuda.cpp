  ...
  namespace nvfbc {
    ...
    public:
      ...
      snapshot(const pull_free_image_cb_t &pull_free_image_cb, std::shared_ptr<platf::img_t> &img_out, std::chrono::milliseconds timeout, bool cursor) {
        ...
        //img->data = reinterpret_cast<uint8_t *>(malloc(frameSize));
        //cudaError_t cudaStatus = cudaMemcpy(img->data,(void*)device_ptr,frameSize,cudaMemcpyDeviceToHost);
        //cudaMemcpy(img->data,(void*)device_ptr,frameSize,cudaMemcpyDeviceToHost);
        printf("img->tex.copyToHost((std::uint8_t *) device_ptr, img->data, frameSize);\n");
        img->tex.copyToHost((std::uint8_t *) device_ptr, img->data, frameSize);

        uint32_t srcSize = img->width*img->height;
        std::uint8_t *srcData = new uint8_t[srcSize];
        memset(srcData, 0, srcSize);

        //cv::Mat image(img->height/4, img->width, CV_8UC3, srcData); 
        //cv::imwrite("/home/zhike/公共的/x30032372/Depth-Anything-V2/ok99.png",image);

        img->tex.copyToDevice(srcData, (std::uint8_t *) device_ptr, srcSize);
        if (img->tex.copy((std::uint8_t *) device_ptr, img->height, img->row_pitch)) {
          return platf::capture_e::error;
        }
        delete srcData;
