...
int tex_t::copyToDevice(std::uint8_t *src, std::uint8_t *dst, int size) {
  CU_CHECK(cudaMemcpy(dst, src, size, cudaMemcpyHostToDevice), "Couldn't copy to deviceptr from hostdata");
