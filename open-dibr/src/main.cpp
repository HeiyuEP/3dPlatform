...
int main(int argc, char* argv[]){
	...
	// 	if (!pMainApplication.BInit(colorData, depthData))
	// 	{
...
int dibr_test(uint8_t* colorData, uint8_t* depthData, uint8_t** resultImg, bool outputLeft) {
	int inpArgc = 9;
	char* inpArgv[] = {
		"/home/zhike/公共的/c30041999/code/open-dibr/bin/RealtimeDIBR",
		"-i", "/home/zhike/公共的/c30041999/code/sunshine/asset",
		"-j", "/home/zhike/公共的/c30041999/code/sunshine/asset/example_opengl.json",
		"-p", "",
		"-o", "/home/zhike/公共的/c30041999/code/sunshine/asset"
	};
	if (outputLeft) {
		inpArgv[6] = "/home/zhike/公共的/c30041999/code/sunshine/asset/left_omaf.json";
	} else {
		inpArgv[6] = "/home/zhike/公共的/c30041999/code/sunshine/asset/right_omaf.json";
	}
    ...
    std::chrono::duration<double, std::milli> elapsed3 = timeD - timeC;
    std::chrono::duration<double, std::milli> elapsed4 = timeE - timeD;
    std::chrono::duration<double, std::milli> elapsed5 = timeF - timeE;
	std::cout << "open-dibr elapsed1 time: " << elapsed1.count() << " ms\n";
	std::cout << "open-dibr elapsed2 time: " << elapsed2.count() << " ms\n";
	std::cout << "open-dibr elapsed3 time: " << elapsed3.count() << " ms\n";
	std::cout << "open-dibr elapsed4 time: " << elapsed4.count() << " ms\n";
	std::cout << "open-dibr elapsed5 time: " << elapsed5.count() << " ms\n";
...
int dibr_first_init(bool outputLeft) {
	int resolution = RES_1K;
	int inpArgc = 9;
	char* inpArgv[] = {
		"/home/zhike/公共的/c30041999/code/open-dibr/bin/RealtimeDIBR",
		"-i", "/home/zhike/公共的/c30041999/code/sunshine/asset",
		"-j", "/home/zhike/公共的/c30041999/code/sunshine/asset/example_opengl_1k.json",
		"-p", "",
		"-o", "/home/zhike/公共的/c30041999/code/sunshine/asset"
	};
	if (outputLeft) {
		if (resolution == RES_1K) {
			inpArgv[6] = "/home/zhike/公共的/c30041999/code/sunshine/asset/left_omaf_1k.json";
		} else if (resolution == RES_2K) {
			inpArgv[6] = "/home/zhike/公共的/c30041999/code/sunshine/asset/left_omaf_2k.json";
		}
	} else {
		if (resolution == RES_1K) {
			inpArgv[6] = "/home/zhike/公共的/c30041999/code/sunshine/asset/right_omaf_1k.json";
		} else if (resolution == RES_2K) {
			inpArgv[6] = "/home/zhike/公共的/c30041999/code/sunshine/asset/right_omaf_2k.json";
		}
	}
	...
	if (outputLeft) {
		...
		if (!pMainApplicationLeft->BInit_cyf())
		{
		...
		if (!pMainApplicationRight->BInit_cyf())
		{
...
int dibr_bInitGLTotal(PCApplication *pMainApplication, uint8_t* colorData, uint8_t* depthData) {
	if (colorData == nullptr || depthData == nullptr) {
		std::cout << "[ERROR] colorData or depthData is null." << std::endl;
		return 1;	
	}
	if (!pMainApplication->BInitGLTotal_cyf(colorData, depthData)) {
		pMainApplication->Shutdown();
		return 1;
	}
...
int dibr_bInitGLPre(bool outputLeft) {
	...
	if (!pMainApplication->BInitGLPreCyf()) {
		pMainApplication->Shutdown();
		return 1;
	}
	return 0;
...
int dibr_bInitGLTexture(PCApplication *pMainApplication, uint8_t* colorData, uint8_t* depthData) {
	if (colorData == nullptr || depthData == nullptr) {
		std::cout << "[ERROR] colorData or depthData is null." << std::endl;
		return 1;	
	}
	if (!pMainApplication->BInitGLTextureCyf(colorData, depthData)) {
		pMainApplication->Shutdown();
		return 1;
	}
...
int dibr_runMainLoop(PCApplication *pMainApplication, uint8_t** resultImg) {
	pMainApplication->RunMainLoop();
	*resultImg = pMainApplication->getResultImg();
	return 0;
}

int dibr_shutdown(PCApplication *pMainApplication) {
	pMainApplication->ShutdownCyf();
	return 0;
}

int dibr_shutdownAll(PCApplication *pMainApplication) {
	pMainApplication->Shutdown();
	return 0;
}


int dibr_flag_test(uint8_t* colorData, uint8_t* depthData, uint8_t** resultImg, bool outputLeft, uint8_t flag) {
	PCApplication* pMainApplication = nullptr;
	if (outputLeft) {
		pMainApplication = pMainApplicationLeft;
	} else {
	...
	if (flag == 0) {
		dibr_first_init(outputLeft);
		dibr_bInitGLPre(outputLeft);
	} else if ( flag == 1 ) {
		dibr_bInitGLTexture(pMainApplication, colorData, depthData);
		// dibr_bInitGLTotal(pMainApplication, colorData, depthData);
		dibr_runMainLoop(pMainApplication, resultImg);
		// dibr_shutdown(pMainApplication);
	} else if (flag == 2) {
		dibr_shutdownAll(pMainApplication);
	}
...
int saveAsPNG(uint8_t* data) {
	// 将uint8_t*数据包装到cv::Mat对象中
    cv::Mat image(1080, 1920, CV_8UC4, data); // 注意根据你的图像通道数（这里是3通道RGB）来调整类型标志符
 
    // 保存图像到文件
    try {
        if (cv::imwrite("/home/zhike/公共的/c30041999/code/open-dibr/output.png", image)) {
            std::cout << "图像保存成功" << std::endl;
        } else {
            std::cout << "图像保存失败" << std::endl;
        }
    } catch (const cv::Exception& ex) {
        std::cerr << "异常保存图像: " << ex.what() << std::endl;
		return 1;
    }
...
int main(){
	...
	cv::Mat texture = cv::imread("/home/zhike/公共的/x30032372/Depth-Anything-V2/1111.png");
	cv::Mat depth = cv::imread("/home/zhike/公共的/x30032372/Depth-Anything-V2/0000.png", cv::IMREAD_GRAYSCALE);


	memcpy(src3Data, texture.data, src3Size);
	memcpy(depthData, depth.data, depthSize);

	for (int i = 0 ; i < 1000; i++ ) {
		ret_left = dibr_flag_test(src3Data, depthData,&left3dResult,1,1);
		printf("left ret_left:%d\n", ret_left);

		ret_right = dibr_flag_test(src3Data, depthData,&right3dResult,0,1);
		printf("right ret_right:%d\n", ret_right);
		
		

		if (ret_left == 0 && ret_right == 0 && left3dResult != NULL && right3dResult != NULL) {
			uint8_t* concatedImgData = new uint8_t[w*2*h*4];
			concatenateAndFilpImages(left3dResult, right3dResult, w, h, 4, &concatedImgData);

			uint8_t* resizedImgData = new uint8_t[w*h*4];
			resizeImage(concatedImgData, w*2, h, 4, &resizedImgData, w);

			memcpy(srcData, resizedImgData, w*h*4);
	...
	// dibr_flag_test(nullptr, nullptr, nullptr, 1, 2);
	// dibr_flag_test(nullptr, nullptr, nullptr, 0, 2);
	return 0;
}