...
public:
	...
	virtual bool BInitGLPreCyf();
	virtual bool BInitGLTextureCyf(uint8_t* colorData, uint8_t* depthData);
	
	virtual void Shutdown();
	virtual void ShutdownCyf();
...
bool Application::BInit(uint8_t* colorData, uint8_t* depthData)
{
	...
	std::cout << "Binit->BInitGL1 time: " << elapsed1.count() << " ms\n";
	std::cout << "Binit->BInitGL2 time: " << elapsed2.count() << " ms\n";
...
bool Application::BInitGL(uint8_t* colorData, uint8_t* depthData)
{
	...
	std::chrono::duration<double, std::milli> elapsed1 = timeB - timeA;
    std::chrono::duration<double, std::milli> elapsed2 = timeC - timeB;
	std::cout << "BInitGL1 time: " << elapsed1.count() << " ms\n";
	std::cout << "BInitGL2 time: " << elapsed2.count() << " ms\n";

	return true;
}

bool Application::BInitGLPreCyf()
{
	printf("bool Application::BInitGLCyf() start\n");
	printf("inputCameras.size:%d\n", inputCameras.size());
	int luma_height = inputCameras[0].res_y;
	printf("bool Application::BInitGLCyf() a\n");

	int luma_height_rounded = ((luma_height + 16 - 1) / 16) * 16; //round luma height up to multiple of 16
	int texture_height = luma_height_rounded + /*chroma height */luma_height / 2;
	float chroma_offset = float(luma_height_rounded - luma_height);
	printf("bool Application::BInitGLCyf() 1\n");
	SetupCameras(); // needs to go first
	printf("bool Application::BInitGLCyf() 2\n");
	SetupStereoRenderTargets();
	printf("bool Application::BInitGLCyf() 3\n");
	if (!CreateAllShaders(1.0))
		return false;
	printf("bool Application::BInitGLCyf() end\n");
...
bool Application::BInitGLTextureCyf(uint8_t* colorData, uint8_t* depthData)
{
	// int time = 5;
	// std::cout << "0开始睡眠" << time << "秒" << std::endl;
    // // 睡眠1秒
    // std::this_thread::sleep_for(std::chrono::seconds(time));
    // std::cout << "0醒来~" << std::endl;
	...
	// std::cout << "1开始睡眠" << time << "秒" << std::endl;
    // // 睡眠1秒
    // std::this_thread::sleep_for(std::chrono::seconds(time));
    // std::cout << "1醒来~" << std::endl;

	SetupCompanionWindow();

	// std::cout << "2开始睡眠" << time << "秒" << std::endl;
    // // 睡眠1秒
    // std::this_thread::sleep_for(std::chrono::seconds(time));
    // std::cout << "2醒来~" << std::endl;
...
void Application::ShutdownCyf()
{
	...
	if (textures_color != NULL) {
		printf("-----textures_color != NULL-----\n");
		
		glDeleteTextures((GLsizei)inputCameras.size(), textures_color);
		delete[] textures_color;
	}
	if (textures_depth != NULL) {
		printf("-----textures_depth != NULL-----\n");
...
void Application::RunMainLoop()
{
	...
	if (!options.asap) {
	...
	else if (options.saveOutputImages) { // 走这个分支
		for (int frame = 0; frame < options.outputNrFrames; frame++) {
			for (int i = 0; i < outputCameras.size(); i++) {
				...
				// int time = 3;
				// std::cout << "0RunMainLoop-0开始睡眠" << time << "秒" << std::endl;
				// // 睡眠1秒
				// std::this_thread::sleep_for(std::chrono::seconds(time));
				// std::cout << "0RunMainLoop-0醒来~" << std::endl;

				RenderFrame(frame > 0 && i == 0, outputCameras[i].name, frame);

				// std::cout << "0RunMainLoop-1开始睡眠" << time << "秒" << std::endl;
				// // 睡眠1秒
				// std::this_thread::sleep_for(std::chrono::seconds(time));
				// std::cout << "0RunMainLoop-1醒来~" << std::endl;
			}
		}
	}
	else {
...
bool Application::RenderFrame(bool nextVideoFrame, std::string outputCameraName, int frameNr)  // 增加4M
{
	auto timeA = std::chrono::high_resolution_clock::now();
    ...
    auto timeB = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> elapsed1 = timeB - timeA;
	std::cout << "RenderTarget time: " << elapsed1.count() << " ms\n";
...
bool Application::SetupRGBTextures(uint8_t* colorData, uint8_t* depthData) {
	...
	for (int i = 0; i < inputCameras.size(); i++) {
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		// std::cout << "0-0开始睡眠" << time << "秒" << std::endl;
		// // 睡眠1秒
		// std::this_thread::sleep_for(std::chrono::seconds(time));
		// std::cout << "0-0醒来~" << std::endl;
		...
		if (inputCameras[i].bitdepth_color > 8) {
			...
			std::cout << "inputCameras[i].bitdepth_color <= 8" << std::endl;
		...
		// std::cout << "0-1开始睡眠" << time << "秒" << std::endl;
		// // 睡眠1秒
		// std::this_thread::sleep_for(std::chrono::seconds(time));
		// std::cout << "0-1醒来~" << std::endl;
		...
		// std::cout << "0-2开始睡眠" << time << "秒" << std::endl;
		// // 睡眠1秒
		// std::this_thread::sleep_for(std::chrono::seconds(time));
		// std::cout << "0-2醒来~" << std::endl;
		if (inputCameras[i].bitdepth_depth > 8) {
			...
			std::cout << "inputCameras[i].bitdepth_depth <= 8" << std::endl;
