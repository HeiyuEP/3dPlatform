...
int MAX_CAPTURE_FRAME_COUNT = 3;
static int depth_to_dibr_num = 0;
static int src_rgb_width = 1920;
static int src_rgb_height = 1080;
static int src_rgb_channel = 3;
static int src_rgba_channel = 4;
...
namespace video {
  struct depth_image_input {
    uint8_t *src_data = nullptr;
    int src_size;

    uint8_t *src_rgb_data = nullptr;
    int src_rgb_size;

    uint8_t *depth_data = nullptr;
    int depth_size;
  };
  ...
  void concatenate_filp_images(uint8_t* img_data_left, uint8_t* img_data_right, img_size single_width_img, uint8_t** new_data) {
    int width = single_width_img.width;
    int height = single_width_img.height;
    int channels = single_width_img.channel;
    
    int new_width = width * 2; // 左右图拼接,宽度一致因此乘2
    *new_data = new uint8_t[new_width * height * channels];
    ...
    for (int y = 0; y < height; ++y) {
      ...
      int ret = memcpy_s(new_row, width*channels, row_left, width*channels); // 左半部分
      if (ret != EOK) {
        BOOST_LOG(error) << "Row left image memcpy failed! error code: " << ret;
        return;
      }

      ret = memcpy_s(new_row+(width*channels), width*channels, row_right, width*channels); // 右半部分
      if (ret != EOK) {
        BOOST_LOG(error) << "Row right image memcpy failed! error code: " << ret;
        return;
      }
    }
  }

  void resize_image(uint8_t* input_data, uint8_t** output_data, img_size single_width_img) {
  ...
  encode_avcodec(int64_t frame_nr, avcodec_encode_session_t &session, safe::mail_raw_t::queue_t<packet_t> &packets, void *channel_data, std::optional<std::chrono::steady_clock::time_point> frame_timestamp) {
    ...
    while (ret >= 0) {
      ...
      if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
        BOOST_LOG(debug) << "encode_avcodec: AVERROR(EAGAIN) or AVERROR_EOF";
        auto time_b = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> time_cost_encode = time_b - time_a;
        BOOST_LOG(debug) << "time_cost_encode: " << time_cost_encode.count() << "ms";
        return 0;
      }
      else if (ret < 0) {
        return ret;
  ...
  int
  get_depth_image_input(
    depth_image_input &depth_image_input) {
    int size = src_rgb_width * src_rgb_height;

    size_t src_size = size * src_rgba_channel;
    std::uint8_t *src_data = new uint8_t[src_size];
    if (src_data == nullptr) {
      BOOST_LOG(error) << "Src image data alloc failed!";
      return -1;
    }

    size_t src_rgb_size = size * src_rgb_channel;
    std::uint8_t *src_rgb_data = new uint8_t[src_rgb_size];
    if (src_rgb_data == nullptr) {
      free_img_data(src_data);
      BOOST_LOG(error) << "src rgb image temp data alloc failed!";
      return -1;
    }

    size_t depth_size = size;
    std::uint8_t *depth_data = new uint8_t[depth_size];
    if (depth_data == nullptr) {
      free_img_data(src_rgb_data);
      free_img_data(src_data);
      BOOST_LOG(error) << "depth image data alloc failed!";
      return -1;
    }

    depth_image_input.src_data = src_data;
    depth_image_input.src_size = src_size;
    depth_image_input.src_rgb_data = src_rgb_data;
    depth_image_input.src_rgb_size = src_rgb_size;
    depth_image_input.depth_data = depth_data;
    depth_image_input.depth_size = depth_size;

    return 0;
  }

  void
  free_depth_image_input(
    depth_image_input &depth_image_input) {
    if (depth_image_input.depth_data != nullptr) {
        delete depth_image_input.depth_data;
        depth_image_input.depth_data = nullptr;
    }
    if (depth_image_input.src_data != nullptr) {
        delete depth_image_input.src_data;
        depth_image_input.src_data = nullptr;
    }
    if (depth_image_input.src_rgb_data != nullptr) {
        delete depth_image_input.src_rgb_data;
        depth_image_input.src_rgb_data = nullptr;
    }
  }
  ...
  get_img(
    ...
    std::shared_ptr<platf::img_t> depth_image,
    std::shared_ptr<platf::img_t> src_rgb_image,
    depth_image_input depth_image_input) {
    if(img == nullptr || depth_image == nullptr || src_rgb_image == nullptr) {
      BOOST_LOG(error) << "error addr!";
      return -1;
    }

    depth_image->width = img->width;
    depth_image->height = img->height;
    depth_image->pixel_pitch = 1; // 单通道灰度图
    depth_image->row_pitch = img->width * depth_image->pixel_pitch;
    depth_image->frame_timestamp = img->frame_timestamp;
    int ret = memcpy_s(depth_image->data, depth_image_input.depth_size, depth_image_input.depth_data, depth_image_input.depth_size);
    if (ret != EOK) {
      BOOST_LOG(error) << "Depth image data memcpy failed! error code: " << ret;
      return -1;
    }

    src_rgb_image->width = img->width;
    src_rgb_image->height = img->height;
    src_rgb_image->pixel_pitch = src_rgb_channel;
    src_rgb_image->row_pitch = img->width * src_rgb_image->pixel_pitch;
    src_rgb_image->frame_timestamp = img->frame_timestamp;
    ret = memcpy_s(src_rgb_image->data, depth_image_input.src_rgb_size, depth_image_input.src_rgb_data, depth_image_input.src_rgb_size);
    if (ret != EOK) {
      BOOST_LOG(error) << "src rgb image data memcpy failed! error code: " << ret;
      return -1;
    }
  ...
  int
  execute_depth_model(
    GenerateDepthImg *depth_model,
    std::shared_ptr<platf::img_t> img,
    img_event_t depth_images,
    img_event_t src_rgb_images,
    depth_image_input depth_image_input) { 
    int ret = -1;
    std::shared_ptr<platf::img_t> depth_image = std::make_shared<platf::img_t>();
    std::shared_ptr<platf::img_t> src_rgb_image = std::make_shared<platf::img_t>();
    depth_image->data = new uint8_t[depth_image_input.depth_size];
    if (depth_image->data == nullptr) {
      BOOST_LOG(error) << "depth image data alloc failed!";
      return -1;
    }
    src_rgb_image->data = new uint8_t[depth_image_input.src_rgb_size];
    if (src_rgb_image->data == nullptr) {
      BOOST_LOG(error) << "src rgb image data alloc failed!";
      free_img_data(depth_image->data);
      return -1;
    }
    if(img->data != nullptr) {
        ret = memcpy_s(depth_image_input.src_data, depth_image_input.src_size, img->data, depth_image_input.src_size);
        if (ret != EOK) {
          BOOST_LOG(error) << "Src image data memcpy failed! error code: " << ret;
          free_img_data(depth_image->data);
          free_img_data(src_rgb_image->data);
          return -1;
        }
    }
    depth_model->Execute(depth_image_input.src_data, depth_image_input.src_rgb_data, depth_image_input.depth_data);
    ret = get_img(img, depth_image, src_rgb_image, depth_image_input);
    if (ret != 0) {
      BOOST_LOG(error) << "get img failed! error code: " << ret;
      free_img_data(depth_image->data);
      free_img_data(src_rgb_image->data);
      return -1;
    }
    std::lock_guard<std::mutex> lock(queueMutex);
    depth_images->raise(depth_image);
    src_rgb_images->raise(src_rgb_image);
    depth_to_dibr_num++;
  ...
  get_depth_img(
    ...
    GenerateDepthImg *depth_model = new GenerateDepthImg(src_rgb_width, src_rgb_height, src_rgba_channel);
    if (depth_model == nullptr) {
      BOOST_LOG(error) << "GenerateDepthImg init failed!";
      return;
    }

    int ret = get_depth_image_input(depth_image_input);
    if (ret != 0) {
      BOOST_LOG(error) << "Depth image data memcpy failed! error code: " << ret;
      goto CLEANUP;
    }

    while(depth_to_dibr_num < MAX_CAPTURE_FRAME_COUNT && images->running()) {
      auto time_a = std::chrono::high_resolution_clock::now();
      if (auto img = images->pop(minimum_frame_time)) 
      {
        ret = execute_depth_model(depth_model, img, depth_images, src_rgb_images, depth_image_input);
        if (ret != 0) {
          BOOST_LOG(error) << "execute depth model failed! error code: " << ret;
          goto CLEANUP;
        }
      }
      auto time_b = std::chrono::high_resolution_clock::now();
      time_cost_get_depth_img = time_b - time_a;
      BOOST_LOG(debug) << "time_cost_get_depth_img: " << time_cost_get_depth_img.count(); 
    }

    CLEANUP: {
      free_depth_image_input(depth_image_input);
      delete depth_model;
      depth_model = nullptr;
      BOOST_LOG(info) << "free depth_model"; 
    }
  }

  void
  get_3D_img_do_render(
    img_event_t res_images,
    std::shared_ptr<platf::img_t> depth_image,
    std::shared_ptr<platf::img_t> src_rgb_image) {
    int ret = -1;
    uint8_t* left_3d_result = nullptr;
    int ret_left = DIBRFlagTest(src_rgb_image->data, depth_image->data, &left_3d_result, 1, 1);
    uint8_t* right_3d_result = nullptr;
    int ret_right = DIBRFlagTest(src_rgb_image->data, depth_image->data, &right_3d_result, 0, 1);
    delete src_rgb_image->data;
    delete depth_image->data;
    if (ret_left == 0 && ret_right == 0 && left_3d_result != nullptr && right_3d_result != nullptr) {
      img_size single_width_img;
      single_width_img.width = src_rgb_width;
      single_width_img.height = src_rgb_height;
      single_width_img.channel = src_rgba_channel;
      ...
      resize_image(concated_img_data, &resized_img_data, single_width_img);
      std::shared_ptr<platf::img_t> res_img = std::make_shared<platf::img_t>();
      res_img->data = new uint8_t[src_rgb_image->width * src_rgb_image->height * src_rgba_channel];
      if (res_img->data == nullptr) {
        BOOST_LOG(error) << "res image data alloc failed!";
        goto CLEANUP;
      }
      res_img->width = src_rgb_image->width;
      res_img->height = src_rgb_image->height;
      res_img->pixel_pitch = src_rgba_channel;
      res_img->row_pitch = src_rgb_image->width * src_rgba_channel;
      res_img->frame_timestamp = src_rgb_image->frame_timestamp;
      ret = memcpy_s(
        res_img->data, src_rgb_image->width * src_rgb_image->height * src_rgba_channel, 
        resized_img_data, src_rgb_image->width * src_rgb_image->height * src_rgba_channel);
      if (ret != EOK) {
        BOOST_LOG(error) << "3D image data memcpy failed! error code: " << ret;
        goto CLEANUP;
      }
      res_images->raise(res_img);
      CLEANUP: {
  ...
  get_3D_img(
    ...
    while (true) {
      ...
      data_condition.wait(lock, [] { return (depth_to_dibr_num > 0); });
      while (depth_to_dibr_num > 0) {
        auto time_a = std::chrono::high_resolution_clock::now();
        auto depth_image = depth_images->pop(minimum_frame_time);
        auto src_rgb_image = src_rgb_images->pop(minimum_frame_time);
        lock.unlock();
        depth_to_dibr_num--;
        if (depth_image->data == nullptr || src_rgb_image->data == nullptr) {
          BOOST_LOG(error) << "depth_image->data or src_rgb_image->data is nullptr";
          break;
        }
        
        get_3D_img_do_render(res_images, depth_image, src_rgb_image);

        auto time_b = std::chrono::high_resolution_clock::now();
        time_cost_get_3D_img = time_b - time_a;
        BOOST_LOG(debug) << "[INFO] time_cost_get_3D_img: " << time_cost_get_3D_img.count() << " ms";
        lock.lock();
      }
    }
  }
  ...
  encode_run(
    ...
    while (true) {
      ...
      if (encode(frame_nr++, *session, packets, channel_data, frame_timestamp)) {
        ...
        std::chrono::duration<double, std::milli> time_period = current - previous;
        BOOST_LOG(debug) << "encode gap: " << time_period.count() << "ms";
      }
  ...
  capture_async(
    ...
    while (!shutdown_event->peek() && images->running()) {
      ...
      auto depth_images = std::make_shared<img_event_t::element_type>();
      auto src_rgb_images = std::make_shared<img_event_t::element_type>();
      auto res_images = std::make_shared<img_event_t::element_type>();

      std::thread get_depth_img_thread(get_depth_img, std::ref(images),std::ref(depth_images), std::ref(src_rgb_images), config);
      get_depth_img_thread.detach();

      std::thread dibr_thread(get_3D_img, std::ref(mail), std::ref(res_images),std::ref(depth_images), std::ref(src_rgb_images), config);
      dibr_thread.detach();
